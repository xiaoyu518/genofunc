...
def filter_fasta(in_dir,in_metadata,gene_list,min_length,symmetric,out_dir,log_file):
    ...
    for files in fasta_files:
        max_len = 0
        output_file = out_dir + files[files.rfind("/")+1:]+"_filtered.fasta"
        out_f = open(output_file,"w")
        for record in SeqIO.parse(files, "fasta"):
            if len(record.seq) > max_len:
                max_len = len(record.seq)
        min_length = math.trunc(min_length*max_len)
        ...
        for record in SeqIO.parse(files, "fasta"):
            if len(record.seq) >= min_length:
                if record.id.find("|") > -1:
                    ...
                    gene_dic[gene].append(record.id)
